...
**Date:** 2026-04-16
**Subject:** `docs/history/2026-04-16-design-wu-039-protocol-core.md`
...
- **What:** `docs/releases/v0.2.0/track-0-shared.md` line 15 enumerates the types WU-039 delivers; the list explicitly includes `ConnectionReady` (20 types). The design doc (`2026-04-16-design-wu-039-protocol-core.md`) and `internal/protocol/messages.go` both ship 19 types and do not declare `ConnectionReady` / `MethodConnectionReady` / `connection.ready`.
- **Evidence:**
  - `docs/releases/v0.2.0/track-0-shared.md:15` — "… `ConnectionPing`, `ConnectionHealth`, `ConnectionReady`"
  - `docs/features/0008-bff-server.md:211` — "Note: `connection.ping`, `connection.health`, `connection.ready`, and `session.sync` are harness→server messages listed in the harness table above."
  - `docs/features/0008-bff-server.md:411` and `:445` — `connection.ready` is invoked during auto-start probing and is described as a simplified boolean readiness check.
...
- **Evidence:**
  ...
  - `docs/features/0008-bff-server.md` §"Framing" — no cap stated (grepped).
  - `docs/releases/v0.2.0/track-0-shared.md` WU-039 spec — no cap stated.
...
- **Recommended disposition:** **fix in same WU** (cheap) — add three bullet references in the package godoc: (a) `// Feature spec: docs/features/0008-bff-server.md (Protocol Specification section)`; (b) `// JSON-RPC 2.0: https://www.jsonrpc.org/specification`; (c) a one-line forward pointer to the `*_test.go` conformance work in WU-093. This is a doc-only change, low risk.
