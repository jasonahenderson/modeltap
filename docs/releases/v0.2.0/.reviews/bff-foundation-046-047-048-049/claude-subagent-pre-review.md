...
**Date:** 2026-04-16
**Subject:** `docs/history/2026-04-16-design-bff-foundation-046-047-048-049.md`
...
- **Evidence:**
  - `docs/features/0008-bff-server.md:416-417` — "Harness sends `connection.ping` every 15 seconds"
  - `docs/features/0008-bff-server.md:418` — "Server replies with `connection.pong` within 5 seconds"
  - `docs/features/0008-bff-server.md:420` — "Server-side: after heartbeat timeout (missed pings for 30 seconds)..." — the server is missing *pings from the harness*, not *pongs from the harness*
  - `docs/history/2026-04-16-design-bff-foundation-046-047-048-049.md:390-410` — D4.5: "server sends `connection.ping` notifications... The harness responds with `connection.ping` requests (pong)."
...
- **What:** The design defines `CapabilitiesRegisterResponse` in `internal/bff/capabilities.go` (D5.2) with fields `{negotiated_version, server_version, max_frame_size, max_attachment_size}`. The protocol-types bundle (`docs/history/2026-04-16-design-protocol-types-040-041-093.md` line 279) defines `CapabilitiesRegisterResponse` in `internal/protocol/tools.go` with fields `{registered []ToolDefinition, server_capabilities ServerCapabilities, rejected []RejectedTool}`. These are two different types with the same name in two different packages, carrying entirely different fields and semantics. The design explicitly says "This type lives in `internal/bff/capabilities.go` (not in `internal/protocol/`) because it is server-specific", but the protocol-types bundle already defines a wire type with this name.
- **Evidence:**
  - `docs/history/2026-04-16-design-bff-foundation-046-047-048-049.md:499-507` — BFF `CapabilitiesRegisterResponse` with 4 fields
  - `docs/history/2026-04-16-design-protocol-types-040-041-093.md:279` — Protocol `CapabilitiesRegisterResponse` with `registered`, `server_capabilities`, `rejected`
  - `docs/history/2026-04-16-design-protocol-types-040-041-093.md:322` — `ServerCapabilities` carries `protocol_version` and `protocol_version_range`
...
- **Evidence:**
  - `docs/history/2026-04-16-design-bff-foundation-046-047-048-049.md:409` — `return &protocol.ConnectionPong{Time: time.Now().UTC()}, nil`
  - `docs/history/2026-04-16-design-protocol-types-040-041-093.md:269` — `ConnectionPong | connection.pong | No fields`
...
- **Evidence:**
  - `docs/releases/v0.2.0/track-a-bff-server.md:52` — "handles `capabilities.register`, `capabilities.update`, `capabilities.request`"
  - `docs/features/0008-bff-server.md:207` — `capabilities.request` is server-to-harness
  - `docs/history/2026-04-16-design-bff-foundation-046-047-048-049.md:449-553` — D5 covers register and update only
...
- **Evidence:**
  - `docs/history/2026-04-16-design-bff-foundation-046-047-048-049.md:272-309` — `ConnState` iota with `String()` method
  - `docs/features/0008-bff-server.md:379-395` — canonical state names
...
- **Evidence:**
  - `docs/history/2026-04-16-design-bff-foundation-046-047-048-049.md:383` — "connection.ping (in any state after ConnConnecting)"
  - `docs/features/0008-bff-server.md:159` — connection not established until auth + registration complete
  - `docs/history/2026-04-16-design-bff-foundation-046-047-048-049.md:396-400` — heartbeat monitor checks `lastPong` but initialization not specified
...
- **Evidence:**
  - `docs/history/2026-04-16-design-bff-foundation-046-047-048-049.md:430-431` — `ReadyResponse{Ready: conn.State() == ConnReady}`
  - `docs/features/0008-bff-server.md:445` — requires auth + storage + capabilities + routing healthy
...
- **Evidence:**
  - `docs/history/2026-04-16-design-bff-foundation-046-047-048-049.md:418-425` — partial field list
  - `docs/history/2026-04-16-design-protocol-types-040-041-093.md:317` — full `HealthResponse` type
  - `docs/features/0008-bff-server.md:422-443` — full example
...
- **Evidence:**
  - `docs/history/2026-04-16-design-bff-foundation-046-047-048-049.md:139-155` — D2.5 `ValidateTurnSubmit` in transport.go
  - `docs/releases/v0.2.0/track-a-bff-server.md:19-22` — inherited validation requirement
...
- **Evidence:**
  - `docs/features/0008-bff-server.md:456-458` — "After re-establishing the connection (auth + capabilities), the harness sends `session.sync`"
  - `docs/history/2026-04-16-design-bff-foundation-046-047-048-049.md:369-385` — dispatch gate rules
...
- **Evidence:**
  - `docs/history/2026-04-16-design-bff-foundation-046-047-048-049.md:439` — `store.ReleaseSessionLock(sessionID, conn.id)`
  - `docs/history/2026-04-16-design-bff-foundation-046-047-048-049.md:649` — dependencies list: `Store` interface (session lock methods)
