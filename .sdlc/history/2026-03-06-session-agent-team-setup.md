...
- Every action must be logged to `.sdlc/history/`, with plans before and summaries after
- Discussed orchestration approach: incremental work units (primary) with status-based resumption (safety net)
- Discussed stopping/resuming across sessions — `.sdlc/history/status.md` serves as the resumption mechanism
...
- `.sdlc/features/web-dashboard.md` — Web dashboard feature spec (status: accepted)
- `.sdlc/history/status.md` — Initialized project status file
...
2. **Status-based resumption** — `.sdlc/history/status.md` as safety net for interrupted sessions
...
- TPM to create master plan (`.sdlc/history/plan.md`) from accepted ADRs and features
