...
The existing proxy core (HTTP forwarding, SSE capture, provider parsing, metrics) remains valuable as the mechanism for communicating with providers. The runtime server is a new layer on top that manages conversations and serves the harness.

## Solution

Add a runtime server (orchestration runtime) layer to the modeltap server that accepts connections from the harness via a purpose-built protocol, manages conversation state, and uses the existing proxy core to communicate with providers. The runtime server is the harness's only interface to model providers — the harness never speaks provider-specific protocols.
...
The harness treats the runtime server connection as a managed lifecycle, not a raw socket.
...
The runtime server is started via existing service management (FEAT-0004) or inline:
...
8. The existing proxy functionality (capture, metrics, retention) continues to work — the runtime server layer does not break the v1 proxy core.
...
10. The harness auto-starts or reconnects to the local runtime server in the solo profile without user action when the service is installed but stopped.
...
13. Reconnection after harness crash, runtime server crash, or network drop preserves session identity. `session.sync` reports the final state of any in-flight turn.
...
| ADR | Relationship |
|-----|-------------|
| ADR-0001 (Go) | runtime server is implemented in Go, same binary |
...
| ADR-0005 (Capture) | Full capture continues — runtime server adds conversation-level capture alongside request-level |
...
| ADR-0009 (MCP) | MCP stdio interface is separate and unchanged. The runtime server does not use MCP for harness communication. |

## Parallel Build Strategy

FEAT-0008 (Runtime Server) and FEAT-0009 (Terminal Harness) can be built in parallel. The protocol contract defined in this spec (Protocol Specification, Protocol Messages, Protocol Payload Schemas, Canonical Field Names, Tool Catalog Schema) is the integration surface. Both teams build against test doubles of the other side.

**Runtime server team** builds against a test harness: a Go client that sends protocol messages, validates responses, and simulates tool execution. Tests cover: connection lifecycle, turn streaming, tool call round-trips, session persistence, model routing, compaction, multi-model branching, and all diagnostic codes.
...
**Status:** planned. Implementation tracked under `PATCH-0017` (`.sdlc/patches/0017-session-scoped-project-context.md`). Driven by `FEAT-0023` (Desktop GUI Client), which is the first client to require multiple concurrent chats on one connection.
