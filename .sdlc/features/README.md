...
| [FEAT-0008](0008-runtime-server.md) | Runtime Server | proposed |
...
| [FEAT-0025](0025-reasoning-content-display.md) | Reasoning content streaming, persistence, and display | draft |

Other previously feature-shaped docs were reclassified when they proved to be upstream explorations or implementation-scoped patches rather than active behavior contracts. See `.sdlc/explorations/` and `.sdlc/patches/`.
...
3. **Accept** — Status `accepted`. The feature is selected for implementation. Work units are planned in `.sdlc/releases/<version>/` and tracked through completion. Only `accepted` features drive work per `CLAUDE.md`.
...
Review artifacts (canonical findings, plan reviews, syntheses) live under `.sdlc/features/.reviews/`. See `.reviews/README.md` for the layout.
