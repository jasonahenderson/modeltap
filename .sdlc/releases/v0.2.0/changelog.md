...
| [PATCH-0016](../../patches/0016-pr1-ci-test-failures-triage.md) | Fix v0.2.x test suite failures and lint regressions surfaced by PR #1 CI | done | Shipped: pin `:memory:` SQLite pool to one connection, drop racy `s.ln=nil` in harness mockBFF, bound `BashTool` timeout cleanup with `cmd.WaitDelay`, replace bash brace expansion with portable `awk` in truncation test, rename deprecated `reflect.Ptr` to `reflect.Pointer` for govet `inline`. Unblocks PR #1's CI on Linux. Merged via PR #2. |
