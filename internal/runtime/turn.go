...
func handleTurnSubmit(ctx context.Context, conn *Connection, params json.RawMessage) (any, error) {
	...
	// a new UUID on startup but the Runtime may not have seen it yet.
	if submit.SessionID == "" {
	...
	if isMulti {
		...
		// Runtime level. The use case is handled by sub-agents (FEAT-0013):
		...
		return nil, &TransportError{
			...
			Message: "multi-model routing is not implemented at the Runtime level; use sub-agents (FEAT-0013) for parallel model execution",
		}
	}
	modelName := models[0]
