...
func newCfgWithRuntime(t *testing.T, sock string) *config.Config {
	t.Helper()
...
func TestStartRuntimeServer_NoEndpoints_WarnsButStarts(t *testing.T) {
	sock := shortSocketPath(t)
	cfg := newCfgWithRuntime(t, sock)

	var stderr bytes.Buffer
	srv, err := startRuntimeServer(cfg, newCfgStore(t), &stderr)
	if err != nil {
...
func TestStartRuntimeServer_EndpointsRegistered(t *testing.T) {
	sock := shortSocketPath(t)
	cfg := newCfgWithRuntime(t, sock)
	cfg.Providers = map[string]config.ProviderConfig{
		// proxy-only entry — should be skipped by the Runtime.
		"anthropic": {Upstream: "https://api.anthropic.com"},
		// Runtime endpoint entries.
...
func TestStartRuntimeServer_InvalidEndpointTypeReported(t *testing.T) {
	sock := shortSocketPath(t)
	cfg := newCfgWithRuntime(t, sock)
	cfg.Providers = map[string]config.ProviderConfig{
	...
	if !strings.Contains(stderr.String(), "runtime provider \"weird\"") {
		t.Errorf("expected provider error in stderr; got %q", stderr.String())
	}
}

func TestStartRuntimeServer_RoutingTreeAndManualModels(t *testing.T) {
	sock := shortSocketPath(t)
	cfg := newCfgWithRuntime(t, sock)
	cfg.Providers = map[string]config.ProviderConfig{
		"ollama-local": {Type: "ollama", Host: "http://localhost:11434"},
	}
	cfg.Runtime.Models = map[string]config.ModelOverrideConfig{
		"llama-3.1-8b": {
	...
	cfg.Runtime.Routing = map[string]any{
...
// TestResolveProviderHost pins PATCH-0005: the Runtime's provider
